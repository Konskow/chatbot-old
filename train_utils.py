...
def input_tensor(sentences: List[str], vocabulary: Vocabulary) -> (torch.Tensor, torch.Tensor):
    ...
    padded_tensor = torch.tensor(padded_batch).to(dtype=torch.long)
    return padded_tensor, lengths


# Returns padded target sequence tensor, padding mask, and max target length
def output_tensor(sentences: List[str], vocabulary: Vocabulary) -> (torch.Tensor, torch.Tensor, torch.Tensor):
    indices_batch = [indices_from_sentence(vocabulary, sentence) for sentence in sentences]
    ...
    mask = torch.tensor(mask).to(dtype=torch.uint8)
    padded_tensor = torch.tensor(padded_list).to(dtype=torch.long)
